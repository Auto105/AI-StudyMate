...
export function quizPrompt(text: string) {
  return {
    system: [
      ...
      '자료에 없는 내용을 새로 지어내지 마세요.',
      '객관식(mcq)은 정확히 3개 이상, OX(ox)는 정확히 2개 이상 생성하세요.',
      '각 객관식은 options를 4개 가지며, 정답은 그중 하나여야 합니다.',
      '반드시 JSON 객체만 반환하세요.',
      '스키마:',
      '{',
      '  "mcq": [',
      ...
      '  ]',
      '}',
      'mcq 필드명은 options(choices 아님), ox 필드명은 question(statement 아님)을 사용하세요.',
